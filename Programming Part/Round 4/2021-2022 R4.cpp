...
// Round to even numbers in this problem when numbers that are *exactly* halfway between two integers.
double round_even(double num) {
...
int main() {
	for(int n = 0; n < 5; ++n) {
		...
		for(int i = 0;;) {
			++i;
			z = func(z, C);
			z = { round_even(z[0]*100.0)/100.0, round_even(z[1]*100.0)/100.0 };
			...
			auto p = make_pair( z[0]*100.0, z[1]*100.0 );
